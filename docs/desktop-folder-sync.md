...
未变化文件不会重复调用后端导入接口；后端仍会用来源哈希、路径、文件名和 SHA-256 做第二层幂等校验。文件仅改名但字节未变化时，仍会更新原映射、笔记标题和来源路径，而不是生成重复笔记。
...
4. **保留 Nowen 编辑并停止跟踪**
   - 保留用户在 Nowen 中的正文和附件
   - 移除该笔记的同步元信息与服务端来源映射
   - 自动把该源文件的精确相对路径加入当前目录的排除规则
   - 后续扫描不再导入该路径；源文件不会被修改
   - 若排除规则已满，会退化为“停止覆盖并提示”，避免解除映射后又被下一轮重新导入

该功能不是双向同步。冲突副本和停止跟踪后的笔记都不会被写回本地目录。
...
支持 `*`、`**` 和 `?`。同步根目录还可创建 `.nowenignore`，语法与设置页相同。选择冲突策略“停止跟踪”后，系统可能自动追加具体文件路径；用户可以在设置页查看和移除。
